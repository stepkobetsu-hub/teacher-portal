...
const sheet={
 ...
 getRange(row,col,height=1,width=1){return {
  ...
  getDisplayValues:()=>Array.from({length:height},(_,i)=>rows[row-1+i].slice(col-1,col-1+width).map(v=>String(v).replace(/^'(\d+)$/,'$1'))),
...
test('add bank later; zeros preserved; immutable and unrelated fields preserved',()=>{
 ...
 assert.equal(rows[94][7],'0001');for(const i of [0,3,6,13,16,35])assert.equal(rows[95][i],orig[i]);
...
test('logout invalidates token; no personal profile fields in properties',()=>{
 ...
 for(const privateValue of ['新住所','test@example.invalid','001234567890','0203','試験'])assert(!stored.includes(privateValue));
});
